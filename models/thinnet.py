...
__all__ = ['ThinNet', 'ThinNetv3']

class ConvBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, stride: int = 1, padding: int = 1, bn: bool = True):
...
class DuplicateConvBlock(nn.Module):
    def __init__(self, channels: int, kernel_size: int = 3, stride: int = 1, padding: int = 1, n_layers: int = 1, bn: bool = True):
...
class ThinNetv3(nn.Module):
    def __init__(self, in_channels: int = 1, n_classes: int = 10, filters: int = 32):
        super().__init__()
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, filters, 3, padding=0, stride=1, bias=0), 
            nn.ReLU(inplace=True), 
        ...
        self.avg = nn.AdaptiveAvgPool2d((2, 2))
        self.fc = nn.Linear(self.filters * 2 * 2, self.n_classes)
    ...
    def forward(self, x):
        ...
        for idx in range(self.filters):
            # print(x[:, idx, :, :].unsqueeze(1).shape)
            # print(idx)
            out.append(self.convs[idx](x[:, idx, :, :].unsqueeze(1)))
